...
while minScore ~= oldScore
    ...
    for i = 1:d
        ...
        % if the new score is better than the previous one then stores 
        % minScore and minInd if not do nothing. So we choose the index
        % that improves the most the score. If no index improves the score
        % then would not change minScore and exit the wile loop.
        if score < minScore
            minInd = ind_new;
            minScore = score;
        end
